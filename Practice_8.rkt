...
(define (parse-sexpr sexpr)
  (match sexpr
    ...
    [(list 'if0 cond second third)
     (If0 (parse-sexpr cond) (parse-sexpr second) (parse-sexpr third))]
...
(define Y-combinator (parse "{fun {f}
                                  {call {fun {x} {fun {n} {call {call x x} n}}}
...
(define (preprocess expr de-env)
  ...
  (cases expr
    ...
    [(WRec bound-id named-expr bound-body)
     (sub (With bound-id 
                (Call Y-combinator (list (Fun (list bound-id) named-expr)))
                bound-body))]
    ...
    [(Call fun-expr arg-exprs)
     ...
     (if (= 1 (length arg-exprs))
       ...
       (sub (Call (Call fun-expr (list (first arg-exprs)))
                  (rest arg-exprs))))]
    [(If0 cond second third)
     (CIf0 (sub cond) (sub second) (sub third))]))
     ...
     (let ([fval (eval fun-expr env)])
       (cases fval
         ...
         [else (error 'eval "`call' expects a function, got: ~s"
                            fval)]))]
    [(CIf0 cond second third)
     (if (equal? (NumV 0) (eval cond env)) 
         (eval second env) 
         (eval third env))]))
